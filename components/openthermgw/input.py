...
OpenThermGatewayInput = generate.openthermgw_ns.class_("OpenThermGatewayInput")
...
def input_schema(entity: schema.InputSchema) -> cv.Schema:
    schema = cv.Schema({
        ...
        cv.Optional(CONF_step, 1.): cv.float_,
    })
...
def generate_setters(entity: cg.MockObj, conf: Dict[str, Any]) -> None:
    ...
    generate.add_property_set(entity, CONF_step, conf)
