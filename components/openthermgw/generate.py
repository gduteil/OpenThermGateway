...
def define_has_component(component_type: str, keys: List[str]) -> None:
    cg.add_define(
        f"OPENTHERMGW_{component_type.upper()}_LIST(F, sep)", 
        cg.RawExpression(" sep ".join(map(lambda key: f"F({key}_{component_type.lower()})", keys)))
    )
    for key in keys:
        cg.add_define(f"OPENTHERMGW_HAS_{component_type.upper()}_{key}")
...
def define_message_handler(component_type: str, keys: List[str], schema_: schema.Schema[TSchema]) -> None:
    ...
    cg.add_define(
        f"OPENTHERMGW_{component_type.upper()}_MESSAGE_HANDLERS(MESSAGE, ENTITY, entity_sep, postscript, msg_sep)",
        cg.RawExpression(
            " msg_sep ".join([ 
                f"MESSAGE({msg}) "
...
async def component_to_code(component_type: str, schema_: schema.Schema[TSchema], type: cg.MockObjClass, create: Create, config: Dict[str, Any]) -> List[str]:
    ...
    """
    cg.add_define(f"OPENTHERMGW_USE_{component_type.upper()}")

    hub = await cg.get_variable(config[const.CONF_OPENTHERMGW_ID])
